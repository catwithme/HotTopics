...
# --- 数据抓取 (V4.0 核心修改 fetch_weibo_top) ---
...
    base_weibo_url = "https://s.weibo.com/weibo?q=" # 用于构造新的搜索链接
    
    try:
...
            title = it.get('title') # 已经是截断后的标题
            url = it.get('url') # 已经是处理后的链接
            
            # --- 核心健壮性检查 ---
...
            # 将末尾的 '  ' 换成 '\n'，确保钉钉消息中每条热搜独占一行
            parts.append(f"{i}. [{escaped_title}]({url})\n") 
    
    # B站部分
    if bilibili:
        parts.append("\n# B站热榜（Top {}）\n".format(len(bilibili)))
